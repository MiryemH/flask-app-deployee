...
from ModeleDeBDD import db, User, Role, app
...
if __name__ == "__main__":            
    with app.app_context():
        ...
        mod_role = Role(nom='Modérateur')
        user_role = Role(nom='User')
        db.session.add_all([admin_role,mod_role,user_role])
        db.session.commit()
        #Il ne faut pas oublier d'assigner un role au user
        john.role = admin_role
        susan.role = user_role
        david.role = user_role
        magda.role = mod_role
        anna = User(nom='Anna', mdp='secret')
        anna.role = admin_role
        db.session.add(anna)
        db.session.commit() 
        
        #Afficher les utilisateurs ayant le role admin
        for user in admin_role.users:
            print(user)

        print(User.query.filter_by(role=admin_role).all())
        
        #Afficher les users ayant le role user_role
        users_a_role_u = User.query.filter(User.role == user_role).all()
        print(users_a_role_u)
        
        #Afficher le nom du role d'un user
        user1 = User.query.filter_by(nom_user="Magda").first()
        print(user1.role.nom)  
